...
- 啟動預設肥/英與 `Shift+Space` 半全形切換設定，支援按住 Shift 連按 Space 連續切換半/全
...
平常重建並啟動 `bin\Debug\uclliu.exe` 可直接執行：

```bat
build_and_run.bat
```

此腳本只停止 executable path 完全等於本 repo `bin\Debug\uclliu.exe` 的程序，不影響其他目錄的同名程式；建置失敗不會啟動舊檔。建置成功後會以 `bin\Debug` 為 working directory 隱藏啟動，並回報 PID 與檔案版本。
...
`build_tsf.bat` 會編譯 `tsf_bridge\UclTsfBridge\UclTsfBridge.vcxproj` 的 `Release|x64` 與 `Release|Win32`，並複製到 `tsf_bridge\x64`、`tsf_bridge\x86`、`bin\Debug\tsf_bridge` 與 `bin\Release\tsf_bridge`。需要 Visual Studio 2022/2026 或 C++ Build Tools、MSVC x64/x86 toolset 與 Windows SDK。若 DLL 正被瀏覽器、Explorer、Codex、Notepad++ 等程序載入，請先切離 TSF Bridge 或關閉相關程序再重跑。
...
| `build_and_run.bat` | 精準停止、重建並啟動 `bin\Debug\uclliu.exe` |
