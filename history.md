...
## 2026-05-26 - OpenCC Lite s2t 移植

### 任務目標

1. 評估 OpenCC 是否可在不新增 NuGet / DLL 的前提下移植到 C# 版。
2. 將 Python 版 `,,,z` 使用的 `s2t` 詞彙級簡轉繁補回 C# 版。
3. 補上 OpenCC License、NOTICE 與使用方式，避免發行時遺漏第三方授權。

### 實作紀錄

- 新增 `OpenCcLite.cs`，以純 C# 載入 OpenCC `STPhrases.txt` / `STCharacters.txt`，採詞彙優先、最長匹配、候選取第一值的 `s2t` 流程。
- 新增 `third_party/opencc`，保存 `s2t.json`、`STPhrases.txt`、`STCharacters.txt`、`LICENSE.txt`、`NOTICE.txt` 與使用說明。
- 字典以 Embedded Resource 包入 exe，執行時不需要 NuGet，也不需要額外 DLL。
- 啟動後背景預熱 OpenCC Lite，避免第一次使用 `,,,z` 時才在 UI 流程解析字典。
- `,,,z` 反白文字轉字根前改用 `simple2trad_opencc()`，OpenCC Lite 失敗時才 fallback 到舊逐字 `simple2trad()`。
- README / CHANGELOG / GOALS 補上 OpenCC Lite 狀態、授權與發行注意事項。

### 驗證紀錄

- 先新增核心測試並確認紅燈：缺少 `OpenCcLite`。
- `dotnet run --project tools\UclLiuCoreTests\UclLiuCoreTests.csproj` 通過。
