...
- 遊戲模式命令由 `,,,lock` / `,,,unlock` 改為 `,,,game` / `,,,normal`，避免與 `,,,l` UI 長版模式衝突。
...
- `liu.cin` / `liu.json` 屬於本機產生或使用者自備的字碼表輸出，不應隨一般功能修正一起進入版本庫。
- `.gitignore` 新增 `/liu.cin` 與 `/liu.json`，保留原始資料表與程式碼變更的 commit 邊界。
