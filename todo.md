...
- [x] 保留逗號上下文，並於閒置 3 分鐘後結束上下文。
- [x] 實作一般輸入自動學習 1～3 字上下文，例如 `王 -> 小明`。
- [x] 實作智慧字根候選排序，常選字逐步往前。

### 個人記憶

- [x] 以 `candidate_memory.json` 保存個人詞組與字根選字統計。
- [x] 記憶體累積變更，停止輸入 3 分鐘或正常離開時保存。
- [x] 使用暫存檔完成後替換，損壞檔備份為 `.broken`。
- [x] 加入清除智慧選字記憶功能，不刪官方候選表。

### 選單與 UI

- [x] 新增 `12. 候選字相關`，將離開移到 `13. 離開(Quit)`。
- [x] 缺少候選表時只顯示 `請先下載候選字`，開啟專案 GitHub。
- [x] 候選表存在時顯示總開關、連續出字、智慧字根與清除記憶。
