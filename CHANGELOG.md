...
- 修正 `,,,x` / `,,,z` 框選轉換流程，優先用 UI Automation 讀取選取文字，再 fallback 到 `WM_COPY` 與安全版 `Ctrl+C`，並延後到 keyboard hook 回傳後執行，避免讀到舊剪貼簿或尾鍵繼續流入目標視窗。
- `,,,x` / `,,,z` 在 Notepad++ / Scintilla 控制項新增 `SCI_COPY` 專用取字路徑，避免框選文字沒有先手動複製就取不到。
