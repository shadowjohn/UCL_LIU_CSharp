...
- 右下角選單在一般使用者權限時新增「★以系統管理員身分重新啟動肥米」，方便接續 TSF Bridge 註冊與設定流程。
- 切換 `TSF出字模式` 前會先檢查 TSF Bridge DLL、註冊狀態與目前權限；未符合條件時會導向管理員重啟或註冊流程，不再默默切到 TSF 模式。

### Fixed

- 修正注音查詢候選鍵盤行為：候選出現後空白可送出第一候選，`Shift+Space` 在有下一頁時會先換頁，不會直接切半全形。
- 修正 `';a` 後按空白查不到 `ㄇ` 這類單獨注音符號候選的問題；混合候選列仍會過濾注音標籤，避免把 `ㄧ` 混入「一、壹、衣」等候選。
